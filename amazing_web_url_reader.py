#!/usr/bin/env python3
"""Amazing Web URL Reader - MCP Server with markdown-first fetching.

This is an advanced MCP (Model Context Protocol) server that first tries to fetch
native markdown directly from the target site and falls back to Playwright when
the site only serves HTML. This allows it to take advantage of markdown-aware
responses when available while still handling JavaScript-heavy sites, SPAs, and
dynamically loaded content that simple HTTP requests would miss.

Features:
---------
- Native markdown negotiation when sites support `text/markdown`
...
- AMAZING_READER_ACCEPT_HEADER: Optional override for the Accept header when fetching pages
  (leave `text/markdown` in the value to keep native markdown negotiation enabled)
...
- Markdown-capable sites can avoid browser rendering entirely; Playwright remains the fallback
...
def _fetch_native_markdown(url: str) -> tuple[str, dict[str, Any]] | None:
    """Fetch markdown directly when the server negotiates a markdown response."""
    accept_header = _get_preferred_accept_header()
    if "text/markdown" not in accept_header.lower():
        logger.debug("Accept header does not include text/markdown; skipping native markdown fetch")
        return None
    ...
    timeout_seconds = max(1.0, get_smart_timeout(url) / 1000)
    ...
    try:
        with urllib_request.urlopen(request, timeout=timeout_seconds) as response:
            ...
            if not content_type.lower().startswith("text/markdown"):
                logger.debug(
                    "Direct fetch returned %s for %s; falling back to Playwright",
            ...
            if markdown_tokens is not None:
                try:
                    meta["markdown_tokens"] = int(markdown_tokens)
                except ValueError:
                    logger.debug("Ignoring non-integer X-Markdown-Tokens value %r", markdown_tokens)

            return markdown_content, meta
    except HTTPError as exc:
        logger.info(
            "Direct markdown fetch returned HTTP %s for %s; falling back to Playwright",
            exc.code,
            url,
        )
    except (URLError, OSError) as exc:
        logger.info("Direct markdown fetch failed for %s; falling back to Playwright: %s", url, exc)
...
def _summarize_single_pass(text: str, target_tokens: int, host: str, model: str) -> str:
    ...
    prompt = (
        ...
        "Output should be well-structured markdown with headings and bullet points "
        "where helpful.\n\n"
...
def _summarize_map_reduce(text: str, target_tokens: int, host: str, model: str) -> tuple[str, dict]:
    """Summarize large text via map-reduce style and return summary plus debug info."""
...
def _html_to_markdown_advanced(html_content: str, url: str | None = None) -> str:
    ...
    # Find the main content area using generic structural selectors.
    main_content = None
...
def _process_element_advanced(element, markdown_parts: list[str], depth: int) -> None:
    ...
    if element.get("style"):
        if "display:none" in element.get("style").replace(
            " ", ""
        ) or "visibility:hidden" in element.get("style").replace(" ", ""):
            return
...
async def handle_list_tools() -> list[types.Tool]:
    ...
    return [
        types.Tool(
            ...
            description=(
                "Read web content by preferring native markdown responses and "
                "falling back to Playwright for JavaScript-heavy pages."
            ),
            inputSchema={
                "type": "object",
                "properties": {
                    ...
                    "wait_time": {
                        "type": "integer",
                        "description": (
                            "Additional milliseconds to wait after page load (default: 2000)"
                        ),
                        "default": 2000,
                    },
                    "scroll_to_bottom": {
                        "type": "boolean",
                        "description": (
                            "Whether to scroll to bottom to trigger lazy-loaded "
                    ...
                    "use_ollama_summarization": {
                        "type": "boolean",
                        "description": (
                            "Enable Ollama-based summarization to target token "
                    ...
                    "ollama_host": {
                        ...
                        "description": (
                            ...
                            "Defaults to OLLAMA_HOST env or "
                    ...
                    "ollama_model": {
                        "type": "string",
                        "description": (
                            "Ollama model name (e.g., gpt-oss:20b). Defaults "
                            "to OLLAMA_MODEL env if set"
                        ),
                    },
                },
                "required": ["url"],
...
async def handle_call_tool(
    ...
    try:
        ...
        try:
            ...
            raise ValueError(f"URL is malformed: '{url}'. {str(e)}") from e

        logger.info("Reading web URL: %s", url)

        render_meta: dict[str, Any] = {"render_method": "playwright"}
        native_markdown = _fetch_native_markdown(url)
        if native_markdown is not None:
            markdown_content, native_meta = native_markdown
        ...
        effective_host = ollama_host_arg or env_ollama_host or "http://localhost:11434"
        ...
        summarization_enabled = bool(
            use_ollama_summarization or (summary_target_tokens and effective_model)
        )
        ...
        if (
            ...
            and effective_model
            and summary_target_tokens
            and summary_target_tokens > 0
        ):
            try:
                ...
                if should_summarize:
                    ...
                    meta["summarization"]["final_estimated_tokens"] = _estimate_tokens_from_text(
                        markdown_content
                    )
            except Exception as e:
            ...
            meta["summarization"]["final_estimated_tokens"] = meta["summarization"][
...
async def main():
    ...
    try:
        ...
        try:
            ...
            fd0_kind = (
                "tty"
                if _stat.S_ISCHR(fd0_mode)
                else "fifo"
                if _stat.S_ISFIFO(fd0_mode)
                else "file"
                if _stat.S_ISREG(fd0_mode)
                else str(fd0_mode)
            )
            fd1_kind = (
                "tty"
                if _stat.S_ISCHR(fd1_mode)
                else "fifo"
                if _stat.S_ISFIFO(fd1_mode)
                else "file"
                if _stat.S_ISREG(fd1_mode)
                else str(fd1_mode)
            )
        except Exception as e:
